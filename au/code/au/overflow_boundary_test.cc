...
TEST(MultiplyTypeBy, MaxGoodForSignedDivByPosIntIsCappedUpperLimitTimesMagInv) {
    struct I8UpperLimit16 : NoLowerLimit<int8_t> {
        static constexpr int8_t upper() { return 16; }
    };

    EXPECT_THAT(max_good_value(multiply_type_by<int8_t>(mag<1>() / mag<2>()), I8UpperLimit16{}),
                SameTypeAndValue(int8_t{32}));

    EXPECT_THAT(max_good_value(multiply_type_by<int8_t>(mag<1>() / mag<7>()), I8UpperLimit16{}),
                SameTypeAndValue(int8_t{112}));

    // Clamped cases.
    EXPECT_THAT(max_good_value(multiply_type_by<int8_t>(mag<1>() / mag<8>()), I8UpperLimit16{}),
                SameTypeAndValue(int8_t{127}));
    EXPECT_THAT(max_good_value(multiply_type_by<int8_t>(mag<1>() / mag<9>()), I8UpperLimit16{}),
                SameTypeAndValue(int8_t{127}));
}

TEST(MultiplyTypeBy, MaxGoodForSignedDivByNegativeIntIsCappedLowerLimitTimesMagInv) {
    struct I8LowerLimitMinus16 : NoUpperLimit<int8_t> {
        static constexpr int8_t lower() { return -16; }
    };

    EXPECT_THAT(
        max_good_value(multiply_type_by<int8_t>(-mag<1>() / mag<2>()), I8LowerLimitMinus16{}),
        SameTypeAndValue(int8_t{32}));

    EXPECT_THAT(
        max_good_value(multiply_type_by<int8_t>(-mag<1>() / mag<7>()), I8LowerLimitMinus16{}),
        SameTypeAndValue(int8_t{112}));

    // Clamped cases.
    EXPECT_THAT(
        max_good_value(multiply_type_by<int8_t>(-mag<1>() / mag<8>()), I8LowerLimitMinus16{}),
        SameTypeAndValue(int8_t{127}));
    EXPECT_THAT(
        max_good_value(multiply_type_by<int8_t>(-mag<1>() / mag<9>()), I8LowerLimitMinus16{}),
        SameTypeAndValue(int8_t{127}));
}

TEST(MultiplyTypeBy, MaxGoodForFloatDivByPosIntIsCappedUpperLimitTimesMagInv) {
    struct FloatUpperLimit64 : NoLowerLimit<float> {
        static constexpr float upper() { return 64.0f; }
    };

    EXPECT_THAT(max_good_value(multiply_type_by<float>(mag<1>() / mag<2>()), FloatUpperLimit64{}),
                SameTypeAndValue(128.0f));

    EXPECT_THAT(max_good_value(multiply_type_by<float>(mag<1>() / mag<8>()), FloatUpperLimit64{}),
                SameTypeAndValue(512.0f));

    // Clamped cases.
    constexpr auto m = mag<64>() / highest_floating_point_as_mag<float>();
    ASSERT_THAT(is_integer(inverse(m)), IsTrue());
    EXPECT_THAT(max_good_value(multiply_type_by<float>(m * mag<2>()), FloatUpperLimit64{}),
                SameTypeAndValue(std::numeric_limits<float>::max() / 2.0f));
    EXPECT_THAT(max_good_value(multiply_type_by<float>(m), FloatUpperLimit64{}),
                SameTypeAndValue(std::numeric_limits<float>::max()));
    EXPECT_THAT(max_good_value(multiply_type_by<float>(m / mag<2>()), FloatUpperLimit64{}),
                SameTypeAndValue(std::numeric_limits<float>::max()));
}

TEST(MultiplyTypeBy, MaxGoodForFloatDivByNegIntIsCappedLowerLimitTimesMagInv) {
    struct FloatLowerLimitMinus64 : NoUpperLimit<float> {
        static constexpr float lower() { return -64.0f; }
    };

    EXPECT_THAT(
        max_good_value(multiply_type_by<float>(-mag<1>() / mag<2>()), FloatLowerLimitMinus64{}),
        SameTypeAndValue(128.0f));

    EXPECT_THAT(
        max_good_value(multiply_type_by<float>(-mag<1>() / mag<8>()), FloatLowerLimitMinus64{}),
        SameTypeAndValue(512.0f));

    // Clamped cases.
    constexpr auto m = mag<64>() / lowest_floating_point_as_mag<float>();
    ASSERT_THAT(is_integer(inverse(m)), IsTrue());
    EXPECT_THAT(max_good_value(multiply_type_by<float>(m * mag<2>()), FloatLowerLimitMinus64{}),
                SameTypeAndValue(std::numeric_limits<float>::max() / 2.0f));
    EXPECT_THAT(max_good_value(multiply_type_by<float>(m), FloatLowerLimitMinus64{}),
                SameTypeAndValue(std::numeric_limits<float>::max()));
    EXPECT_THAT(max_good_value(multiply_type_by<float>(m / mag<2>()), FloatLowerLimitMinus64{}),
                SameTypeAndValue(std::numeric_limits<float>::max()));
}
