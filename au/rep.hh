...
// --- can never be a valid rep, because using it as one would produce nested units.  Screening
// these out also keeps the scalar overloads of `Quantity`'s arithmetic operators from being
// ambiguous with the operators of vector-of-quantity types.
//
// We detect the scalar via `ScalarOf`, so this benefits from all of its automatic probes
// (`::Scalar`, `::value_type`, `.real()`), as well as any user specializations of `ScalarOfTrait`.
template <typename T>
