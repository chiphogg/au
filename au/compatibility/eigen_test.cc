...
TEST(EigenCompatibility, EqualityOfDistinctExpressionRepsWorks) {
    ...
    // templates.  Like heterogeneous addition, this used to fail to compile inside `std::common_type`
    // (undefined for two distinct Eigen expressions).  Unlike order comparisons, equality is
    // well-defined for vectors -- Eigen's `operator==` compares all coefficients -- so it should work.
...
TEST(EigenCompatibility, CrossUnitEqualityOfExpressionRepsWorks) {
    // Operands in *different* units, so one is scaled to the common unit (staying a lazy expression)
    // before the coefficient-wise comparison.  Note that both operands here have the *same*
    // expression rep type: this used to route through the common-rep path and try to `static_cast`
    // one Eigen expression into another (a hard error).  Non-arithmetic reps now always take the
    // `ref_or_scaled_copy` path instead.
