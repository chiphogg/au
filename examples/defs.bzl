...
as linked tabs, so a reader can flip back and forth and compare corresponding lines in place.
...
def single_example(name, expected_output, srcs, deps, hdrs = None):
    ...
    Args:
      ...
      srcs: Sources for the program.
      deps: Deps for the program.
      hdrs: Headers the example defines, if any.  These are compiled as part of the program.
    """
    cc_binary(
        name = name,
        srcs = srcs + (hdrs or []),
