...
using Au).  The website shows them as linked tabs, and the reader flips between them to compare
...
3. Register it in `BUILD.bazel` with `ab_example(...)`, giving the exact expected stdout.  For an
   example with no meaningful plain-C++ counterpart, use `single_example(...)` instead.
...
Two rules for the doc page:

- **Tab labels must be byte-for-byte identical across every page.**  Material's linked tabs match
  on label *text*, so one stray character breaks page-wide flipping.  They are currently
  `⚠️ Before: raw C++` and `✅ After: with Au`.
...
`angular_velocity` exposes a short `headline` region (used by the README and the docs front page)
inside the full `example` region.
...
a ⚠️ has no referent — there is no "before" tab in the reader's codebase.  These files would also be
the only non-ASCII in any C++ source in the repository.  To draw the eye to a specific line, use the
docs layer (`hl_lines`) rather than the source.
...
- **A discussion page on atomic unit conventions** (Hartree vs. Rydberg, and how different
  conventions handle electromagnetic units).  See issue #660.
